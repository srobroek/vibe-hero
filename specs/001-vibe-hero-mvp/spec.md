...
The very first time a user engages vibe-hero (or any time configuration is missing), they are routed into an interactive setup conducted by a dedicated setup skill. It runs a short Q&A on the user's preferences — at minimum: offer cadence (one offer per session vs one per surfaced topic), whether proactive offers are enabled at all, and preferred quiz length — and writes a configuration the rest of the product reads. The tool being learned is **not** asked during setup; it is auto-detected from the host's MCP `clientInfo.name` at connection time (`toolsLearning` remains optional/derived). No quizzes, offers, status, or guidance happen until setup has completed once.

**Why this priority**: This is the front door. Every other behavior (offers, cadence, which curriculum is in focus) depends on configuration existing, and the user explicitly required that setup be the first thing that happens and that it be skill-driven and portable. Without it the product has undefined behavior on first run, so it is P1 and gates the others.
...
- **FR-031**: The product MUST be configured via a dedicated, interactive **setup skill** that conducts a Q&A on user preferences (at minimum: offer cadence per FR-020a, whether proactive offers are enabled, and preferred quiz length) and persists a configuration the rest of the system reads. The tool being learned is **not** asked during setup — it is auto-detected from the host's MCP `clientInfo.name` at connection time; `toolsLearning` in the stored config is optional and derived, not a required setup question. *(Supersedes the original "ask which tool" wording — user-directed change to auto-detection, implemented via MCP handshake `clientInfo`.)* vibe-hero supports ONLY the four enumerated tools (claude-code, codex, kiro-cli, kiro-ide). When the host is an unknown/unsupported tool (e.g. Cursor, Windsurf, Cline, VS Code, or no name provided) AND no `toolsLearning` is explicitly configured, every gated tool MUST return the `UNSUPPORTED_TOOL` sentinel with the raw host name and the supported-tool list; it MUST NOT silently default to claude-code. A user on an explicitly configured `toolsLearning` can override detection.
