...
- Verified mechanics (npx-MCP shape, `${PLUGIN_ROOT}` hook token, MCP declared in `.mcp.json` only with no duplicate in plugin.json, the agentic-packages name-only dep bug) come from the research done before speccing and are reflected in FR-007/008/010/011 and Assumptions.
