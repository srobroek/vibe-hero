...
3. **The Stop hook** — auto-registered from the plugin's `hooks/hooks.json` (`Stop` → `${PLUGIN_ROOT}/scripts/stop-offer.sh`), no settings.json edit (FR-007). The script spawns nothing: it emits an `additionalContext` nudge and the agent calls `get_offer` on the running MCP server (FR-011, agent-mediated).
