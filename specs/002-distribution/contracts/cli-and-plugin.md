...
### `npx -y @vibe-hero/server get-offer --session <id> --tool <toolId>`
- **Behavior**: resolves the end-of-work offer for the session (delegates to the offers engine + profile, per spec-001 `get_offer`), prints one JSON line: `{ offer?: {...} } | { suppressed: "..." }`. Always exits 0 (defensive — never breaks the host).
- **Used by**: the Stop hook (FR-011).
...
3. **The Stop hook** — auto-registered from the plugin's `hooks/hooks.json` (`Stop` → `${PLUGIN_ROOT}/scripts/stop-offer.sh`), no settings.json edit (FR-007).
...
- the `agentic-packages` marketplace (cross-listed by reference, same plugin + same `@vibe-hero/server`, same version — SC-010).
