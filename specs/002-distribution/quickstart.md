...
1. Generate the plugin artifacts (`apm pack` or the repo's generate step) → `.mcp.json` has `mcpServers.vibe-hero` = `npx -y @vibe-hero/server`; `hooks/hooks.json` has a `Stop` hook with a `${PLUGIN_ROOT}` command; `plugin.json` carries identity + skills path and NO name-only deps.
